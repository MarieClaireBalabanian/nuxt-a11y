<template>
  <section ref="blockRef" :class="{paused: paused || isReduced}">
    <div class="marquee-container" ref="marqueeRef" :tabindex="paused || isReduced ? 0 : -1">
      <span
        class="scroll-text"
        :class="[{ animation: startAnimation }]"
        v-for="item in contentMultiplier"
        v-html="text"
      >
    ...
    <button v-if="!isReduced" @click="toggleAnimation">{{ paused ? 'start' : 'stop' }}</button>
  </section>
</template>
    ...
    const text = '<span>one</span><span>one</span><span>one</span>';
        ...
        const checkMultiplier = () => {
            if (marqueeRef.value) {
                ...
                if (children) {
                    contentMultiplier.value = Math.floor((targetWidth / children.offsetWidth) + 1);
                    nextTick(() => {
